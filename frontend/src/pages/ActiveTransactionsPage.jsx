...
const ActiveTransactionsPage = () => {
  ...
  const fetchTransactionsWithPage = async (page, size, term, status, source, policyNo, period) => {
    ...
    setError(null);
    try {
      const response = await PeriodApi.getUnreconciledTransactions(
        page, 
        size, 
        term, 
        status, 
  ...
  if (loading) {
    return (
      <div style={{ padding: '20px', textAlign: 'center' }}>
        <div style={{ fontSize: '18px', color: 'var(--text)' }}>Loading transactions...</div>
      </div>
    );
  }
  ...
  return (
    <div style={{ padding: '20px' }}>
      <div style={{ marginBottom: '20px' }}>
        <h1 style={{ margin: '0 0 20px 0', color: 'var(--text)' }}>Unreconciled Transactions</h1>
...
export default ActiveTransactionsPage;
